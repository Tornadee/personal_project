...
class game:
	...
	def _build_platforms(self):
		...
		while num_platforms < 200:
			num_platforms += 1;
			shift_x = random.randint(-1, 1) * self.deviation;
			shift_z = 5;
			x = round(self.previous_platform_x + shift_x, 1);
			z = round(self.previous_platform_z + shift_z, 1);
	...
	def _return_state(self):
		for i in range(len(self.platforms)):
			...
			if (platform.z > self.player.z + 1):
				...
				'''
				return p_x, p_z#, a_x, a_z, b_x, b_z, rotation of player;
